...
trainDecisionTree maxDepth minPoints dS = let
  ...
  (greater, lesser) = splitAtValue dS bestInd value
  !newL = trainDecisionTreeHelper 1 maxDepth minPoints lesser
  !newR = trainDecisionTreeHelper 1 maxDepth minPoints greater
  res = DecisionTree {
    left = newL,
    right = newR,
    feature = bestInd,
...
trainDecisionTreeHelper :: Int -> Int -> Int -> (DataSet (DV.Vector Double)) -> DecisionTree
trainDecisionTreeHelper currDepth maxDepth minPoints dS = let
  ...
  in if shouldBeLeaf
    then let
      ...
      !newL = trainDecisionTreeHelper newDepth maxDepth minPoints lesser
      !newR = trainDecisionTreeHelper newDepth maxDepth minPoints greater
      res = DecisionTree {
        left = newL,
        right = newR,
        feature = bestInd,
...
findBestSplitRowHelper dS row best@(_, bGini) ind = let
  ...
  (s1@(DataSet {dat=_, labels=l1}), s2@(DataSet {dat=_, labels=l2})) = splitAtValue dS ind value
  thisGini = computeGiniIndexAt l1 l2
...
{-Split a given data set at an index with a threshold. Produces a data set
  ...
  than the threshold.-}
splitAtValue :: (DataSet (DV.Vector Double)) -> Int -> Double -> ((DataSet (DV.Vector Double)), (DataSet (DV.Vector Double)))
splitAtValue dS index thresh = (greaterThanOrEqual, lessThan) where
...
computeGiniIndexAt :: DV.Vector Int -> DV.Vector Int -> Double
computeGiniIndexAt group1 group2 = let
  valueMap = Mp.empty
